...
1. Create 2-3 realistic prompts in `evals/evals.json` or an iteration
   workspace.
2. For new skills, compare `with_skill` against `without_skill`.
3. For existing skills, compare the new version against a snapshot of the old
   skill.
4. Draft objective assertions while runs are in progress.
5. Grade outputs into `grading.json` using fields `text`, `passed`, and
   `evidence`.
6. Aggregate results:
...
If subagents or a browser are unavailable, do not fake benchmark coverage.
Create the prompts and assertions, run what can be run locally, and record the
gap clearly.
